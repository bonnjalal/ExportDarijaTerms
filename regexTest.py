    ...
    def getNoun(self, txt):
        ...
        for l in lines:
            if l.startswith('# ') or l.startswith('#:'):
                ...
                lbMatchList = reH.finditer_with_line_numbers(lbPatt, l)
                uxMatchList = reH.finditer_with_line_numbers(uxPatt, l)
                print(lbMatchList)
                ...
                if bool(lbMatchList):
                    for m in lbMatchList:
                        ...
                        match = m[0]
                        match.replace('|', ' , ')
                        line.replace('{{lb|'+lng+'|'+ m[0] +'}}', match)
                
                if bool(uxMatchList):
                    for m in uxMatchList:
                        match = m[0]
                        line.replace('{{ux|'+lng+'|'+ m[0] +'}}', match)
